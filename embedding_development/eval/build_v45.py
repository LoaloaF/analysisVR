#!/usr/bin/env python3
"""
build_v45.py  —  Replace S69/S70 with joint-GPV + competition figures.

Base: outputs/ultimate_presentation_v43.pptx  (clean, no orphaned parts)
v43 (93) → v45 (93 slides):
  S69: → e07_cue_zone_joint_gpv.png
       title → "Joint GPV in the cue zone: cue+position captures more than marginals"
  S70: → e07_cue_zone_competition.png
       title → "Joint GPV still fails: speed and head angle outcompete cue+position"
"""
import os, zipfile
...
OUTS = [
    os.path.join(root, 'outputs', 'ultimate_presentation_v45.pptx'),
    '/mnt/c/Users/amits/Desktop/ultimate_presentation_v45.pptx',
]

IMG_69 = os.path.join(adir, 'e07_cue_zone_joint_gpv.png')
IMG_70 = os.path.join(adir, 'e07_cue_zone_competition.png')
TARGET_DPI = 200

TITLE_69 = 'Joint GPV in the cue zone: cue+position captures more than marginals'
TITLE_70 = 'Joint GPV still fails: speed and head angle outcompete cue+position'
...
print(f'  S{idx69+1}: joint GPV figure inserted')

# S70 — competition figure
...
print(f'  S{idx70+1}: competition figure inserted')
