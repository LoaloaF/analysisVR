...
fig = plt.figure(figsize=(FIG.FULL[0], FIG.FULL[1]))
...
cb.set_label('Mean pairwise Pearson r\n(cross-seed)', fontsize=FONT.TICK - 1)
cb.ax.tick_params(labelsize=FONT.TICK - 2)

add_footnote(fig,
    f'Metric: mean pairwise Pearson r of predictions on overlapping test trials across seeds; '
    f'each model uses own R²≥{R2_THRESH} mask; hatching = no valid data')
