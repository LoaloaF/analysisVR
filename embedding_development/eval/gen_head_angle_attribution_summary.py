...
gpv_masked = np.where(valid_exp, gpv, np.nan)
ig_masked  = np.where(valid_exp, ig,  np.nan)

gpv_mean = np.nanmean(gpv_masked.reshape(-1, n_groups), axis=0)  # (11,)
ig_mean  = np.nanmean(ig_masked.reshape(-1,  n_groups), axis=0)

n_valid = valid.sum()

# Sort by GPV descending
order = np.argsort(gpv_mean)[::-1]

ha_g  = group_names.index('head_angle')
C_GPV = '#2CA02C'   # green for all GPV bars
C_IG  = '#FF7F0E'   # orange for all IG bars
...
ax.bar(x - width/2, gpv_mean[order], width, color=C_GPV, alpha=0.90, label='GPV (ΔR²)')
ax.bar(x + width/2, ig_mean[order],  width, color=C_IG,  alpha=0.90, label='Mean |IG|')

# Box around head angle
ha_pos = int(np.where(order == ha_g)[0][0])
pad    = 0.05
y_top  = max(gpv_mean[ha_g], ig_mean[ha_g]) * 1.18
y_bot  = min(0, gpv_mean[ha_g], ig_mean[ha_g]) - 0.002
from matplotlib.patches import FancyBboxPatch
ax.add_patch(FancyBboxPatch(
    (ha_pos - width - pad, y_bot),
    2 * width + 2 * pad, y_top - y_bot,
...
ax.set_ylabel('Mean attribution (valid pairs)', fontsize=FONT.LABEL - 1)
...
add_footnote(fig,
    ...
    'Head angle (boxed) is the top-attributed feature by both GPV and IG.')
