...
delta_Vs_mean = delta_Vs["mean"] #/((7*25)/1e3) # 7nA*25 amplification  -> to mV


perc90 = np.percentile(delta_Vs_mean, 90)
plt.hist(delta_Vs_mean, bins=50, )
plt.hist(delta_Vs_mean, bins=50,  density=True, cumulative=True, histtype='step', color='r')
...
plt.savefig(PATH + '/deltaV_hist.png')
plt.show()
...
plt.yscale('log')
plt.savefig(PATH + '/deltaV_std_hist.png')
plt.show()
...
plt.savefig(PATH + '/deltaV_map.png')
plt.show()
...
plt.title("MEA1K: Mean deltaV jump post current stimulation with Ampl=25 (/ 70nA*25 = R)")
cbar = plt.colorbar()
cbar.set_label('deltaV [mV]')
plt.show()