...
def get_data():
    ...
    folder = "/home/vrmaster/Projects/VirtualReality/data/2024-09-13_14-40-46_active/"

    ephys_fname = "ephys_output.raw.h5"
    # behavior_fname = "behavior_2024-08-14_14-31_dummyAnimal_P0800_LinearTrack_3min.hdf5"
    behavior_fname = "2024-09-13_14-41_dummyAnimal_P0500_MotorLearning.xlsx_2min.hdf5"
    ...
    plt.figure()
    unitycam_time = unitycam_data.unitycam_image_pc_timestamp.values
    unity_time = frame_data.frame_pc_timestamp.values
    unity_time_norm = unity_time[1:] - unity_time[1]
    unitycam_time_norm = unitycam_time - unitycam_time[0]
    plt.plot(unity_time_norm - unitycam_time_norm)
    plt.plot(np.diff(unity_time_norm), label='Unity Time Diff')
    plt.plot(np.diff(unitycam_time_norm), label='Unitycam Time Diff')
    plt.legend()
    plt.show()
...
def main():
    ...
    plt.title('Difference between TTL and PC Timestamp before patching: TTL - PC')
    plt.show() 
    
    plt.figure()
    plt.hist(np.diff(frame_pc_timestamp_norm), bins=np.arange(0, 100000, 50))
    plt.yscale('log')
    plt.show()
    
    plt.figure()
    plt.hist(np.diff(frame_ttl_norm), bins=np.arange(0, 100000, 50))
    plt.yscale('log')
    plt.show()
