#!/usr/bin/env python3
"""
For E07 x cue_visible: scatter per-session Cohen's d (neural activity split by
...
print(f'\nE07 x cue_visible — Spearman rho with Cohen d:')
...
for ax, (col, label, color, marker) in zip(axes, methods):
    ...
    ax.set_ylabel("Cohen's d (cue_visible)" if ax is axes[0] else '')
    ax.set_title(f'{label}\nρ={rho:+.3f}  p={p:.3f}', fontsize=10)

plt.suptitle("E07 × cue_visible — attribution vs per-session Cohen's d", fontsize=11)
plt.tight_layout()
for root in (output_dir, desktop_dir):
    plt.savefig(os.path.join(root, 'E07_attribution_vs_cohend.png'), dpi=150, bbox_inches='tight')
plt.close()
print('\nSaved E07_attribution_vs_cohend.png')
