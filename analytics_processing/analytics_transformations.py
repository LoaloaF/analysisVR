...
#         kinematic_cols = ["posbin_position", "posbin_velocity", "posbin_acc",]
...
#             mean_cols = ['frame_position', 'frame_velocity', 'frame_acc',
...
def interp_missing_pos_bins(trial_data):
    # reindex to the full range of spatial bins, then fill NaNs
    from_bin, to_bin = int(trial_data.index[0][1].item()), int(trial_data.index[-1][2])
    midx = pd.MultiIndex.from_arrays([np.ones((to_bin-from_bin)) * trial_data.index[0][0],
                                        np.arange(from_bin, to_bin),
                                        np.arange(from_bin+1, to_bin+1)],
                                        names=['trial_id', 'from_position_bin', 'to_position_bin'])
    # this will create NaN rows for the missing position bins, for interpolation
    data = trial_data.reindex(midx)
    
    # count columns should be filled with 0, animal was too fast
    count_cols = [c for c in data.columns if c.endswith('_count')] + ['nframes_in_posbin']
    data.loc[:, count_cols] = data.loc[:, count_cols].fillna(0)
    
    # Interpolate numeric columns, kinematics 
    # categorical cols
    categorical_cols = [c for c in data.columns if data[c].dtype == 'category']
    data.loc[:,categorical_cols] = data.loc[:,categorical_cols].ffill()
    
    # float columns, interpolate
    float_cols = [c for c in data.columns if data[c].dtype == np.float64]
    data.loc[:,float_cols] = data.loc[:,float_cols].interpolate(method='linear', limit_direction='both')
    
    # int columns, interpolate
    int_cols = [c for c in data.columns if data[c].dtype == np.int64]
    data.loc[:,int_cols] = data.loc[:,int_cols].interpolate(method='linear', limit_direction='both').astype('Int64')
    return data
    
def column2agg_map(columns):
